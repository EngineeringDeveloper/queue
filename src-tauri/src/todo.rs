...
impl LoadedTodo {
  ...
  pub fn update_task(mut self, newTask: Task, taskIndex: usize) {
    println!("{}/n/n{:?}", taskIndex, self.todo_hash[&newTask.priority][taskIndex]);
    
    // change the task to the returned task
    self.todo_hash[&ident_task.priority][taskIndex] = newTask;
